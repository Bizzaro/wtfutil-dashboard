...
func getNextBus(agency string, route string, stopID string, stopTag string) string {
	...
	directionalArray := []Direction{}
	if err := json.Unmarshal(parsedResponse.Predictions.DirectionRaw, &directionalArray); err == nil {
		directionParseType = "array"
	}

	// build the final string
	finalStr := ""

	if directionParseType == "object" {
		predictionParseType := ""
		// hacky, try object parse first
		nextBusObject := Prediction{}
		if err := json.Unmarshal(directional.PredictionRaw, &nextBusObject); err == nil {
			predictionParseType = "object"
		}

		// if object parse failed, it probably means we have an array
		nextBuses := []Prediction{}
		if err := json.Unmarshal(directional.PredictionRaw, &nextBuses); err == nil {
			predictionParseType = "array"
		}

		if predictionParseType == "array" {
	...
	} else if directionParseType == "array" {
		parseType := ""

		for _, directionElement := range directionalArray {
			// hacky, try object parse first
			nextBusObject := Prediction{}
			if err := json.Unmarshal(directionElement.PredictionRaw, &nextBusObject); err == nil {
				parseType = "object"
			}

			// if object parse failed, it probably means we have an array
			nextBuses := []Prediction{}
			if err := json.Unmarshal(directionElement.PredictionRaw, &nextBuses); err == nil {
				parseType = "array"
			}
			if parseType == "array" {
				for _, nextBus := range nextBuses {
